...
- [Local state management](./local-state.html): Learn how to cache and query local alongside remote data using `apollo-link-state`. For a full list of options, check out the API reference for `apollo-link-state`.
